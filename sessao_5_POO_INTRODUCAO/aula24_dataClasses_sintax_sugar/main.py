...
class Pessoa:
    ...
    def __post_init__(self):
        if not isinstance(self.nome, str) and not isinstance(self.sobrenome, str):
            raise TypeError(
                f'Invalid type {type(self.nome).__name__} != str em {self}'
            )
        self.nome = f'{self.nome} {self.sobrenome}'

    @property # Property sendo é usado como um Getter.
...
# Sorted, ao ordenar, ele irá ordenado por ordem alfabética, se quiser por chave, precisa fazer com key e lambda
